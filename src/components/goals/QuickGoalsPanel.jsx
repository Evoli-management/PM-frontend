...
const QuickGoalsPanel = ({ goals, onClose }) => {
  ...
  const renderGoalNode = (goal, level = 0) => {
    const progressColor = goal.progressPercent >= 90 ? 'bg-green-500' : 
                         goal.progressPercent >= 70 ? 'bg-blue-500' : 
                         goal.progressPercent >= 40 ? 'bg-yellow-500' : 'bg-red-500';
    
    return (
      <div key={goal.id} className="mb-3">
        <div className={`flex items-center p-3 rounded-lg ${level > 0 ? 'bg-slate-50' : 'bg-white border border-slate-200'}`}>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          {hierarchy.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-600">Create parent-child relationships between goals to visualize them here.</p>
            </div>
          )}
        </div>
