...
export const PersonalInformation = ({ showToast }) => {
    ...
    const loadUserProfile = async () => {
        try {
            ...
            showToast(t('personalInformation.loadError'), 'error');
        } finally {
    ...
    const validatePersonalForm = () => {
        ...
        if (!personalDraft.firstName.trim()) {
            newErrors.firstName = t('personalInformation.firstNameRequired');
        }
    ...
    const savePersonalInfo = async () => {
        ...
        try {
            ...
            showToast(t('personalInformation.savePersonalSuccess'));
        } catch (error) {
            console.error('Failed to save personal info:', error);
            showToast(t('personalInformation.savePersonalError'), 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleAvatarUpdate = async (avatarData) => {
        try {
            ...
            if (!silent) showToast(t('personalInformation.avatarSuccess'));

            // Dispatch event to notify other components (like Navbar) of profile update
            window.dispatchEvent(new CustomEvent('profileUpdated'));
        } catch (error) {
            console.error('Failed to update avatar:', error);
            showToast(t('personalInformation.avatarError'), 'error');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        return (
            <div className="flex items-center justify-center p-8">
                <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground">{t('personalInformation.loadingProfile')}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            ...
            <Section title={t('personalInformation.personalDetails')} icon="👤">
                ...
                <div className="flex justify-end items-center gap-2 mt-3">
                    {isEditingPersonal ? (
                        <>
                            <LoadingButton variant="outline" onClick={cancelPersonalEdit} disabled={isLoading}>{t('personalInformation.cancel')}</LoadingButton>
                            <LoadingButton onClick={savePersonalInfo} isLoading={isLoading} loadingText={t('personalInformation.saving')}>{t('personalInformation.save')}</LoadingButton>
                        </>
                    ) : (
                        <LoadingButton variant="outline" onClick={() => setIsEditingPersonal(true)}>{t('personalInformation.edit')}</LoadingButton>
                    )}
                </div>
            </Section>
        </div>
