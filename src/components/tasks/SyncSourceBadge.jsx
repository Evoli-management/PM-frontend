import React from "react";

/**
 * SyncSourceBadge
 * Shows a small coloured provider badge on imported tasks.
 * Pass googleId or outlookId to determine which badge to show.
 */
export default function SyncSourceBadge({ googleId, outlookId, style = {} }) {
    if (googleId) {
        return (
            <span
                title="Imported from Google Tasks"
                style={{
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 3,
                    fontSize: 10,
                    fontWeight: 700,
                    padding: "1px 6px",
                    borderRadius: 20,
                    background: "#e8f0fe",
                    color: "#1a73e8",
                    border: "1px solid #d2e3fc",
                    whiteSpace: "nowrap",
                    letterSpacing: 0.2,
                    ...style,
                }}
            >
                <svg width="10" height="10" viewBox="0 0 24 24" style={{ flexShrink: 0 }}>
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                </svg>
                Google
            </span>
        );
    }

    if (outlookId) {
        return (
            <span
                title="Imported from Microsoft To Do"
                style={{
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 3,
                    fontSize: 10,
                    fontWeight: 700,
                    padding: "1px 6px",
                    borderRadius: 20,
                    background: "#e6f4f1",
                    color: "#0078d4",
                    border: "1px solid #c2e0f4",
                    whiteSpace: "nowrap",
                    letterSpacing: 0.2,
                    ...style,
                }}
            >
                <svg width="10" height="10" viewBox="0 0 24 24" style={{ flexShrink: 0 }}>
                    <path fill="#0078d4" d="M11.5 2L2 7v10l9.5 5 9.5-5V7L11.5 2zm0 2.18L19 8.5v7l-7.5 3.95L4 15.5v-7l7.5-4.32z" />
                </svg>
                To Do
            </span>
        );
    }

    return null;
}
